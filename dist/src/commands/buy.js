...
import { PRICE, LABEL, buyConfirmRow } from "../lib/shop.js";
export const data = new SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: `Roll — 💠${PRICE.roll} each`, value: "roll" }, { name: `Claim — 💠${PRICE.claim} each`, value: "claim" }))
...
export async function execute(interaction) {
    ...
    if (balance < total) {
        return interaction.editReply({
            content: `That costs 💠 ${total} — you have 💠 ${balance}, ` +
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(0x5865f2)
        .setDescription(`**${qty} extra ${LABEL[item]}${qty === 1 ? "" : "s"}** for 💠 ${total}\n` +
        `(💠 ${PRICE[item]} each)`)
        .addFields({ name: "Balance now", value: `💠 ${balance}`, inline: true }, { name: "After", value: `💠 ${balance - total}`, inline: true }, {
        name: "Banked",
        value: `${state.bonusRolls} roll(s) · ${state.bonusClaims} claim(s)`,
        inline: true,
    })
        .setFooter({
        text: `Banked ${LABEL[item]}s are spent only after your hourly ` +
