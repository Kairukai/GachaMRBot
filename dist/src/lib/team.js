...
import { TEAM_SIZE, validateTeam, } from "./battle.js";
export const ROLE_CHOICES = ["vanguard", "duelist", "strategist"];
/**
...
export function parseRoles(raw) {
    ...
    const found = raw
        ...
        .filter((part) => ROLE_CHOICES.includes(part));
 ...
 */
export async function setTeam(guildId, userId, slots) {
    ...
    const owned = await db
        ...
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), inArray(schema.claims.cardId, cardIds)));
    const byCard = new Map(owned.map((o) => [o.cardId, o]));
    const notOwned = cardIds.filter((id) => !byCard.has(id));
    if (notOwned.length)
        return { ok: false, notOwned };
    ...
    if (needsRole.length)
        return { ok: false, needsRole };
    const violations = validateTeam(resolved.map((c) => ({
        cardId: c.cardId,
        heroId: c.heroId,
        rarity: c.rarity,
        role: c.role,
    ...
    await db.transaction(async (tx) => {
        ...
        await tx.insert(schema.teamSlots).values(resolved.map((c) => ({
            guildId,
            userId,
            slot: c.slot,
            cardId: c.cardId,
...
export async function clearTeam(guildId, userId) {
 ...
 */
export async function resolveTeam(guildId, userId) {
    ...
    const stillOwned = saved.length
        ? await db
            ...
            .innerJoin(schema.cards, eq(schema.claims.cardId, schema.cards.id))
            .innerJoin(schema.heroes, eq(schema.cards.heroId, schema.heroes.id))
            .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), inArray(schema.claims.cardId, saved.map((s) => s.cardId))))
        : [];
...
export async function teamPickerOptions(guildId, userId, query) {
