...
async function listCostumeTitles() {
    const titles = [];
    let cont;
    do {
        const j = await mw({
...
function parseCostumeInfobox(wikitext) {
...
async function resolveImages(files) {
    ...
    for (let i = 0; i < unique.length; i += 50) {
        ...
        for (const p of Object.values(j.query?.pages ?? {})) {
            ...
            if (url)
                out.set(String(p.title).replace(/^File:/, ""), url);
        }
 ...
 */
async function fetchHeroMeta(names) {
    const out = new Map();
...
async function main() {
    ...
    const parsed = [];
    const skipped = { noInfobox: 0, noHero: 0, noId: 0, badRarity: new Map() };
    ...
    if (skipped.noInfobox)
        console.log(`  skipped ${skipped.noInfobox} without a Costume page infobox`);
    if (skipped.noHero)
        console.log(`  skipped ${skipped.noHero} without a hero field`);
    if (skipped.noId)
        console.log(`  skipped ${skipped.noId} without an id`);
    for (const [k, n] of skipped.badRarity)
        console.log(`  skipped rarity "${k}" ×${n}`);
    const heroMap = new Map();
    for (const p of parsed)
        heroMap.set(slug(p.hero), p.hero);
    ...
    if (missingRole.length) {
        console.log(`  ${missingRole.length} hero(es) without a role: ${missingRole.map((h) => h.name).join(", ")}`);
    }
    /**
    ...
    if (exactDupes)
        console.log(`  collapsed ${exactDupes} duplicate page(s) for the same hero+costume`);
    ...
    const byRarity = cardRows.reduce((a, c) => {
...
//# sourceMappingURL=ingest-wiki.js.map