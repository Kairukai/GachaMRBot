...
import { RARITY_META } from "./gacha.js";
...
export async function rankableOwned(guildId, userId, query) {
    const rows = await db
        .select({
        id: schema.cards.id,
        ...
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), inArray(schema.cards.rarity, ["epic", "legendary"]), sql `${schema.claims.rank} < ${MAX_RANK}`, query
 ...
 */
export async function planFodder(guildId, userId, targetCardId, cost) {
    const owned = await db
        ...
        .innerJoin(schema.cards, eq(schema.claims.cardId, schema.cards.id))
        .innerJoin(schema.heroes, eq(schema.cards.heroId, schema.heroes.id))
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), eq(schema.claims.rank, 1), sql `${schema.claims.cardId} <> ${targetCardId}`))
        .orderBy(asc(schema.cards.id));
    const byRarity = (r) => owned.filter((c) => c.rarity === r);
    const legendaries = byRarity("legendary");
    const rares = byRarity("rare");
    const epics = byRarity("epic");
    const picked = [];
    const take = (row) => picked.push({
        cardId: row.cardId,
        name: row.name,
        hero: row.hero,
        rarity: row.rarity,
    ...
    for (const l of legendaries.slice(0, cost.legendaries))
        take(l);
...
export async function rankState(guildId, userId, cardId) {
    const [row] = await db
        .select({
        rank: schema.claims.rank,
        rarity: schema.cards.rarity,
 ...
 */
export async function rankUp(guildId, userId, targetCardId, fodderCardIds) {
    ...
    if (fodderCardIds.length === 0)
        return fail({ code: "no_fodder" });
    ...
    try {
        return await db.transaction(async (tx) => {
            const [target] = await tx
                ...
                .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), eq(schema.claims.cardId, targetCardId)));
            if (!target)
                abort({ code: "target_not_owned" });
            const rarity = target.rarity;
            if (!RANKABLE.has(rarity))
                abort({ code: "target_not_rankable", rarity });
            if (target.rank >= MAX_RANK)
                abort({ code: "already_max" });
            ...
            const owned = await tx
                ...
                .from(schema.claims)
                .innerJoin(schema.cards, eq(schema.cards.id, schema.claims.cardId))
                .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), inArray(schema.claims.cardId, [...fodderCardIds])));
            ...
            if (ranked.length)
                abort({ code: "fodder_ranked", cardIds: ranked });
            const points = owned.reduce((sum, c) => sum + FODDER_VALUE[c.rarity], 0);
            if (points < cost.points) {
            ...
            const paid = await tx
                ...
                .where(and(eq(schema.users.id, userId), sql `${schema.users.shards} >= ${cost.shards}`))
            ...
            const bumped = await tx
                ...
                .set({ rank: nextRank })
                .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), eq(schema.claims.cardId, targetCardId), eq(schema.claims.rank, target.rank)))
        ...
        if (err instanceof BurnAbort)
            return fail(err.failure);
        throw err;
    }
}
/* ----------------------------------------------------------- interaction */
export function rankUpConfirmRow(cardId, label) {
...
export function explainFailure(f) {
    switch (f.code) {
        ...
        case "fodder_missing":
            return (`Your collection changed while you were deciding — ${f.expected - f.found} of the ` +
...
export async function handleRankUpButton(interaction) {
    ...
    const result = await rankUp(guildId, userId, cardId, plan.cards.map((c) => c.cardId));
    if (!result.ok) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(RARITY_META[state.rarity].color)
        .setDescription(`## ${RARITY_META[state.rarity].emoji} ${state.hero} — ${state.name}\n` +
        `Now **Rank ${result.toRank}**.`)
        .addFields({ name: "Burned", value: `${result.burned} card(s)`, inline: true }, { name: "Shards", value: `💠 ${result.shardsSpent}`, inline: true }, { name: "Balance", value: `💠 ${result.shardBalance}`, inline: true })
        .setFooter({ text: "Burned cards return to the pool for anyone to claim." });
    if (state.image)
        embed.setThumbnail(state.image);
