...
import { consumeBattle, renderMatchEmbed, runChallenge, } from "./challenge.js";
 ...
 * to work.
 */
export async function createChallenge(guildId, challengerId, defenderId, stake) {
    const fail = (failure) => ({ ok: false, failure });
    const existing = await db
        .select({ id: schema.challenges.id })
        .from(schema.challenges)
        .where(and(eq(schema.challenges.guildId, guildId), eq(schema.challenges.challengerId, challengerId), eq(schema.challenges.defenderId, defenderId), eq(schema.challenges.status, "pending")));
    if (existing.length)
        return fail({ code: "already_pending" });
    if (stake.kind === "shards") {
        if (stake.amount <= 0)
            return fail({ code: "stake_too_low" });
        for (const [who, id] of [
    ...
    if (stake.kind === "card") {
        ...
        for (const [who, userId, cardId] of pairs) {
            ...
            if (!owned)
                return fail({ code: "stake_not_owned", who });
        }
    }
    const [row] = await db
        ...
        return await db.transaction(async (tx) => {
            ...
            if (offer.wager === "shards") {
                for (const [who, id] of [
                    ...
                    const [row] = await tx
                        ...
                        .from(schema.users)
                        .where(eq(schema.users.id, id));
                    if ((row?.shards ?? 0) < offer.stakeShards)
                        abort({ code: "shards_gone", who });
                }
            }
            if (offer.wager === "card") {
                const pairs = [
            ...
            if (offer.wager === "shards") {
                ...
                const taken = await tx
                    ...
                    .where(and(eq(schema.users.id, loserId), sql `${schema.users.shards} >= ${offer.stakeShards}`))
                    .returning({ shards: schema.users.shards });
                if (taken.length === 0)
                    abort({ code: "shards_gone", who: loserSide });
                await tx
                    .update(schema.users)
                    .set({ shards: sql `${schema.users.shards} + ${offer.stakeShards}` })
            ...
            if (offer.wager === "card") {
                ...
                const moved = await tx
                    ...
                    .set({ userId: winnerId })
                    .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.cardId, prizeCardId), eq(schema.claims.userId, loserId)))
                    .returning({ id: schema.claims.id });
                if (moved.length !== 1)
                    abort({ code: "stake_gone", who: loserSide });
            }
            await tx
...
export async function closeChallenge(challengeId, status) {
    await db
        .update(schema.challenges)
        .set({ status })
...
/* ---------------------------------------------------------- interaction */
export function challengeButtons(challengeId) {
...
function explainSettleFailure(f) {
    switch (f.code) {
        case "not_pending":
...
async function battlesPerHour(guildId) {
    const [row] = await db
