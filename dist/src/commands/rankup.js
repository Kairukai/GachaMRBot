...
import { RANK_COST, FODDER_VALUE, planFodder, rankState, rankableOwned, rankUpConfirmRow, } from "../lib/rankup.js";
...
export async function autocomplete(interaction) {
    ...
    const choices = await rankableOwned(guildId, interaction.user.id, interaction.options.getFocused());
    return interaction.respond(choices.length
        ? choices
...
export async function execute(interaction) {
    ...
    if (!plan.ok) {
        ...
        lines.push("", `Fodder values — Rare ${FODDER_VALUE.rare}, Epic ${FODDER_VALUE.epic}, ` +
            `Legendary ${FODDER_VALUE.legendary} points. Ranked cards can't be burned.`);
        return interaction.editReply({ content: lines.join("\n") });
    }
    if (balance < cost.shards) {
        return interaction.editReply({
            content: `Rank ${nextRank} costs 💠 ${cost.shards} and you have 💠 ${balance}. ` +
                `Sell some spares or keep rolling.`,
        });
    }
    const counts = plan.cards.reduce((acc, c) => {
        acc[c.rarity] = (acc[c.rarity] ?? 0) + 1;
        return acc;
    }, {});
    const summary = ["rare", "epic", "legendary"]
        .filter((r) => counts[r])
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(meta.color)
        .setDescription(`## ${meta.emoji} ${state.hero} — ${state.name}\n` +
        `Burning **${plan.cards.length} card(s)** (${summary}) and **💠 ${cost.shards}**.\n\n` +
        `${listed}${overflow}`)
        .addFields({ name: "Points", value: `${plan.points}/${cost.points}`, inline: true }, { name: "Shards after", value: `💠 ${balance - cost.shards}`, inline: true }, {
        name: "Ultimate charge",
        value: nextRank === MAX_RANK ? "fully upgraded" : `+${nextRank - 1}/9 of the bonus`,
        inline: true,
    })
        .setFooter({
        text: "This cannot be undone. Burned cards return to the pool for anyone to claim. " +
