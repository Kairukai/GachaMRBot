...
export const LABEL = { roll: "roll", claim: "claim" };
export function buyConfirmRow(item, qty, total) {
 ...
 */
export async function purchase(guildId, userId, item, qty) {
    ...
    `);
        ...
        return {
            ok: true,
            spent: total,
            balance: Number(debited[0].shards),
            rolls: row?.rolls ?? 0,
            claims: row?.claims ?? 0,
        };
    });
...
export async function handleBuyButton(interaction) {
    ...
    const [item, rawQty] = token.split(":");
    const qty = Number(rawQty);
    if ((item !== "roll" && item !== "claim") || !Number.isFinite(qty) || qty < 1)
        return;
    const result = await purchase(guildId, interaction.user.id, item, qty);
    if (!result.ok) {
        return interaction.update({
            content: `Not enough shards — you have 💠 ${result.balance}, this costs ` +
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(0x22c55e)
        .setDescription(`Bought **${qty} ${LABEL[item]}${qty === 1 ? "" : "s"}** for 💠 ${result.spent}.\n` +
        `Balance: **💠 ${result.balance}**`)
        .addFields({ name: "Banked rolls", value: `${result.rolls}`, inline: true }, { name: "Banked claims", value: `${result.claims}`, inline: true })
        .setFooter({
