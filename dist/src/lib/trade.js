...
export function tradeButtons(tradeId) {
 ...
 */
export async function executeSwap(tradeId) {
    return db.transaction(async (tx) => {
        ...
        const give = await tx
            ...
            .where(and(eq(schema.claims.guildId, trade.guildId), eq(schema.claims.cardId, trade.offerCardId), eq(schema.claims.userId, trade.proposerId)))
            .returning({ id: schema.claims.id });
        const take = await tx
            .update(schema.claims)
            .set({ userId: trade.proposerId })
            .where(and(eq(schema.claims.guildId, trade.guildId), eq(schema.claims.cardId, trade.wantCardId), eq(schema.claims.userId, trade.receiverId)))
            .returning({ id: schema.claims.id });
 ...
 */
export async function cardLabel(cardId, guildId) {
    const [c] = await db
        .select({
    ...
    if (guildId) {
        ...
        if (claim && claim.rank > 1)
            rankSuffix = ` · ${rankBadge(claim.rank)}`;
    }
    return `${RARITY_META[c.rarity].emoji} ${c.hero} — ${c.name}${rankSuffix}`;
}
export async function handleTradeButton(interaction) {
    ...
    if (result !== "ok") {
        ...
        await interaction.followUp({
            content: "That trade couldn't complete — one of the cards changed hands since the offer was made.",
            flags: MessageFlags.Ephemeral,
        });
        return;
    }
    const embed = new EmbedBuilder()
        ...
        .setColor(0x22c55e)
        .setDescription(`<@${trade.proposerId}> ⇄ <@${trade.receiverId}>\n\n` +
...
export async function ownedCards(guildId, userId, query) {
    const rows = await db
        .select({
        id: schema.cards.id,
        name: schema.cards.name,
        ...
        .innerJoin(schema.heroes, eq(schema.cards.heroId, schema.heroes.id))
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.userId, userId), query
    ...
    return rows.map((r) => ({
        ...
        name: `${RARITY_META[r.rarity].label}${r.rank > 1 ? ` R${r.rank}` : ""} · ${r.hero} — ${r.name}`.slice(0, 100),
        value: r.id,
    }));
}
