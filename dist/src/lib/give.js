...
export function giveConfirmRow(recipientId, cardId, danger) {
 ...
 */
export async function executeGive(guildId, fromId, toId, cardId) {
    await ensureMember(toId, guildId);
    const moved = await db
        .update(schema.claims)
        .set({ userId: toId, claimedAt: new Date() })
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.cardId, cardId), eq(schema.claims.userId, fromId)))
...
export async function handleGiveButton(interaction) {
    ...
    if (result !== "ok") {
        return interaction.update({
            content: "Nothing was given — you no longer own that card. It may have been traded or sold since the prompt appeared.",
            embeds: [],
            components: [],
        });
    }
    ...
    const meta = RARITY_META[(card?.rarity ?? "rare")];
    ...
    const embed = new EmbedBuilder()
        .setTitle("🎁 Card given")
        .setColor(meta.color)
        .setDescription(`<@${interaction.user.id}> gave **${meta.emoji} ${card?.hero} — ${card?.name}** ` +
        `to <@${recipientId}>`);
    if (card?.image)
