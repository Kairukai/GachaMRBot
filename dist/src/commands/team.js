...
import { TEAM_SIZE, MAX_EPICS } from "../lib/battle.js";
import { ownedCards } from "../lib/trade.js";
import { clearTeam, resolveTeam, setTeam } from "../lib/team.js";
...
export const data = new SlashCommandBuilder()
    ...
    return sub.addStringOption((o) => o
        .setName("wildcard_role")
        .setDescription("Role for a hero that can play more than one (e.g. Deadpool)")
        .setRequired(false)
        .addChoices({ name: "Vanguard", value: "vanguard" }, { name: "Duelist", value: "duelist" }, { name: "Strategist", value: "strategist" }));
})
    .addSubcommand((sub) => sub
    .setName("view")
    .setDescription("Show a line-up.")
    .addUserOption((o) => o.setName("user").setDescription("Whose team to view").setRequired(false)))
...
export async function execute(interaction) {
    ...
    if (sub === "set")
        return setCommand(interaction);
    if (sub === "clear")
        return clearCommand(interaction);
    return viewCommand(interaction);
}
async function setCommand(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const guildId = interaction.guildId;
    const wildcard = interaction.options.getString("wildcard_role");
    const slots = [];
    for (const n of SLOT_OPTIONS) {
    ...
    if (!result.ok) {
        ...
        if ("needsRole" in result) {
            const lines = result.needsRole.map((r) => r.options.length > 1
...
async function viewCommand(interaction) {
    const guildId = interaction.guildId;
    ...
    const body = slots
        .map((s) => {
        if (!s.card)
            return `\`${s.slot}\` ⬜ *Recruit* — empty slot`;
    ...
    const embed = new EmbedBuilder()
        ...
        inline: true,
    }, { name: "Slots filled", value: `${owned}/${TEAM_SIZE}`, inline: true }, { name: "Power", value: `${teamPower(slots.map((s) => s.unit))}`, inline: true });
    if (owned < TEAM_SIZE) {
        embed.setFooter({
...
function renderTeam(username, team) {
    const body = team
        .map((c) => {
