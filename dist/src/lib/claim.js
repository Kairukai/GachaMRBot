...
function rawComponents(interaction) {
    return interaction.message.components.map((c) => c.toJSON());
}
/** Card ids contain a colon (`heroSlug:costumeId`), so only split on the first. */
export function cardIdFromCustomId(customId) {
    return customId.slice(CLAIM_PREFIX.length);
}
export function claimButton(cardId) {
 ...
 */
export function editV2Components(raw, opts) {
    let touched = false;
 ...
 */
function retireButton(interaction, label) {
...
function retireAll(interaction, label) {
    const rows = interaction.message.components.map((r) => ActionRowBuilder.from(r));
    for (const row of rows) {
        for (const c of row.components) {
            c.setDisabled(true).setStyle(ButtonStyle.Secondary);
        }
    }
    if (rows.length === 0)
        return [settledRow(label)];
    return rows;
}
function relative(d) {
...
export async function handleClaim(interaction) {
    ...
    if (age > settings.claimWindowSec * 1000) {
        if (isV2) {
            ...
            if (edited)
                await interaction.update({ components: edited }).catch(() => { });
            return;
        }
    ...
    const embeds = interaction.message.embeds.map((e, i) => i === index
