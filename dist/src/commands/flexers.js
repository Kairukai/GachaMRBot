...
import { PAGE_SIZE, collectorCount, leaderboardPage, memberRank, } from "../lib/leaderboard.js";
...
export async function execute(interaction) {
    ...
    const render = async () => {
        ...
        const lines = rows.map((r, i) => {
            ...
            return (`${badge} <@${r.userId}> — ${r.total} card${r.total === 1 ? "" : "s"}\n` +
        ...
        return new EmbedBuilder()
            ...
            .setColor(0xf59e0b)
            .setFooter({
            text: `Page ${current + 1}/${pages} · ${members} collector${members === 1 ? "" : "s"}` +
                (me ? ` · you're #${me.rank} with ${me.value.toLocaleString()} shards' worth` : ""),
        });
    };
    const nav = (disabled = false) => new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId("prev")
        .setLabel("◀")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(disabled || current === 0), new ButtonBuilder()
        .setCustomId("next")
        .setLabel("▶")
        .setStyle(ButtonStyle.Secondary)
...
//# sourceMappingURL=flexers.js.map