 ...
 */
export async function consumeBattle(userId, guildId, battlesPerHour) {
 ...
 */
export async function runChallenge(guildId, challengerId, defenderId, 
 ...
 */
export function teamPower(units) {
    const s = teamStats(units);
    return Math.round(s.hp + s.dmg * 4 + s.mit * 2 + s.heal * 3);
}
export async function battleRecord(guildId, userId) {
    const [row] = await db
        .select({
        wins: sql `COUNT(*) FILTER (WHERE ${schema.matches.winnerId} = ${userId})::int`,
        total: sql `COUNT(*)::int`,
    })
        .from(schema.matches)
        .where(and(eq(schema.matches.guildId, guildId), sql `(${schema.matches.challengerId} = ${userId} OR ${schema.matches.defenderId} = ${userId})`));
    const wins = Number(row?.wins ?? 0);
    const total = Number(row?.total ?? 0);
    return { wins, losses: total - wins };
}
/** Most recent matches in a guild, for a history view. */
export async function recentMatches(guildId, limit = 5) {
    return db
        .select({
...
function ultLine(fired) {
    return Object.keys(ULT_NAMES)
        .filter((r) => fired[r] > 0)
        .map((r) => ULT_NAMES[r] + (fired[r] > 1 ? " x" + fired[r] : ""))
        .join(", ");
}
function roundLine(r, challenger, defender) {
    ...
    if (a)
        parts.push("**" + challenger + "**: " + a);
    if (b)
        parts.push("**" + defender + "**: " + b);
    return parts.join(" | ");
}
/**
 ...
 */
export function renderMatchEmbed(outcome, challengerName, defenderName, stakeLine) {
    ...
    const log = outcome.result.rounds
        ...
        .join("\n");
    const mvpCard = [...outcome.challenger.slots, ...outcome.defender.slots].find((s) => s.card?.cardId === outcome.result.mvp?.cardId)?.card ?? null;
    const embed = new EmbedBuilder()
        .setTitle(challengerName + " vs " + defenderName)
        .setColor(challengerWon ? 0x22c55e : 0xef4444)
        .setDescription("**" + (challengerWon ? challengerName : defenderName) + " wins** in " +
    ...
    if (stakeLine)
        embed.addFields({ name: "Stake", value: stakeLine });
    return embed;
}
