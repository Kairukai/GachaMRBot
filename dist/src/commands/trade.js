...
import { ownedCards, cardLabel, tradeButtons, TRADE_TTL_MS, } from "../lib/trade.js";
...
export async function autocomplete(interaction) {
    ...
    const target = focused.name === "offer"
        ? interaction.user.id
...
export async function execute(interaction) {
    ...
    const [mine] = await db
        ...
        .from(schema.claims)
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.cardId, offerCardId), eq(schema.claims.userId, interaction.user.id)));
    if (!mine) {
        return interaction.reply({
    ...
    const [theirs] = await db
        .select({ id: schema.claims.id })
        .from(schema.claims)
        .where(and(eq(schema.claims.guildId, guildId), eq(schema.claims.cardId, wantCardId), eq(schema.claims.userId, receiver.id)));
    if (!theirs) {
        return interaction.reply({
