...
let cache: { at: number; counts: Map<Rarity, number> } | null = null;

async function load(): Promise<Map<Rarity, number>> {
  const rows = await db
  ...
  return new Map(rows.filter((r) => r.n > 0).map((r) => [r.rarity as Rarity, r.n]));
}

async function counts(force = false): Promise<Map<Rarity, number>> {
 ...
 */
export async function randomCard(rarity: Rarity) {
