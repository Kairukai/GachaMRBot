import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, SectionBuilder, TextDisplayBuilder, ThumbnailBuilder, MessageFlags, } from "discord.js";
...
export async function execute(interaction) {
    ...
    const gate = await consumeRoll(userId, guildId, settings.rollCooldownSec, settings.rollsPerHour, BATCH);
    if (!gate.ok) {
    ...
    for (const card of cards) {
        ...
        if (owner)
            lines.push(`Owned by <@${owner}>`);
        else if (dupeInBatch)
            lines.push("_Duplicate — already shown above_");
        ...
        if (taken) {
            ...
            container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`💠 +${DUPLICATE_SHARDS[rarity]} shards`));
        }
        else {
            claimable++;
            container.addActionRowComponents(new ActionRowBuilder().addComponents(claimButton(card.id).setLabel(`Claim ${meta.label}`)));
        }
        containers.push(container);
    }
    let balance = null;
    if (shardsEarned > 0)
        balance = await awardShards(userId, shardsEarned);
    const footer = [
    ...
    setTimeout(() => {
        void (async () => {
            ...
            if (!fresh)
                return;
            const edited = editV2Components(fresh.components.map((c) => c.toJSON()), { disableAll: true, label: "Expired" });
            if (edited)
                await message.edit({ components: edited }).catch(() => { });
        })();
    }, settings.claimWindowSec * 1000).unref?.();
}
//# sourceMappingURL=roll5.js.map