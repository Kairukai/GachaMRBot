 ...
 */
import { simulate, seededRng, validateTeam, teamStats, TUNING, } from "../src/lib/battle.js";
const SEEDS = (() => {
...
let uid = 0;
function unit(rarity, role, rank = 1) {
    uid++;
    return {
...
const BUILDS = new Map(ARCHETYPES.map((x) => [x.name, x.build]));
const FIT_CELLS = [
    ...Object.entries(TARGET).flatMap(([rowName, cols]) => Object.entries(cols).map(([colName, want]) => ({
...
function evaluate(t, seeds) {
    ...
    for (const cell of FIT_CELLS) {
        ...
        for (let seed = 1; seed <= seeds; seed++) {
            ...
            if (r.winner === "a")
                wins++;
            if (r.rounds.length >= t.maxRounds)
                stalemates++;
            rounds += r.rounds.length;
            matches++;
            if (aHasLegendary) {
                ultEligible++;
                const fired = r.rounds.some((x) => x.aUlts.vanguard + x.aUlts.duelist + x.aUlts.strategist > 0);
                if (fired)
                    ultSeen++;
            }
        }
    ...
    if (t.power.legendary < t.power.epic)
        cost += 2000;
    /**
...
function fit(iterations, seeds) {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        if (cost < bestCost) {
            ...
            best = candidate;
            if (i % 25 === 0)
                console.log(`  iter ${i}: cost ${cost.toFixed(1)}`);
        }
    }
...
for (const a of ARCHETYPES) {
    ...
    console.log(`  ${pad(a.name, 16)} ${pad(s.hp.toFixed(0), 9)} ${pad(s.dmg.toFixed(1), 8)} ` +
        `${pad(s.heal.toFixed(1), 8)} ${s.mit.toFixed(1)}`);
}
let totalRounds = 0;
let matches = 0;
let hitCap = 0;
function matrix(title, group) {
    const width = 17;
    console.log(`\n${title}\n`);
    ...
    for (const row of group) {
        ...
        for (const col of group) {
            ...
            for (let seed = 1; seed <= SEEDS; seed++) {
                ...
                matches++;
                if (r.rounds.length >= active.maxRounds)
                    hitCap++;
            }
...
console.log(`Hit the ${active.maxRounds}-round cap: ${hitCap}/${matches}` +
    (hitCap === 0 ? "  (no stalemates)" : "  <-- escalation is not ending fights"));
//# sourceMappingURL=battle-sim.js.map