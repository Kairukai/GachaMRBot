 ...
 */
function normaliseRarity(raw: unknown): Rarity | null {
...
type HeroRow = typeof schema.heroes.$inferInsert;
type CardRow = typeof schema.cards.$inferInsert;

async function main() {
  console.log("Fetching heroes…");
  const heroPayload = await api<unknown>("/heroes");
  ...
  const costumePayload = await api<unknown>("/costumes");
  ...
  const unknownRarities = new Map<string, number>();
  let orphaned = 0;
  ...
  for (const c of rawCostumes) {
    ...
    if (id == null || name == null || heroId == null) continue;
    ...
    cardRows.push({
      ...
      imageUrl: absoluteImage(
        pick(c, "icon", "imageUrl", "image_url", "image", "full_image"),
      ),
    });
  }

  console.log(`  → ${cardRows.length} costumes usable as cards`);
  if (orphaned) console.log(`  → ${orphaned} skipped (hero not in roster)`);
  if (unknownRarities.size) {
    console.warn("  → skipped, unrecognised rarity values:");
    for (const [k, n] of unknownRarities) console.warn(`     ${k} ×${n}`);
    console.warn("     Add these to normaliseRarity() in scripts/ingest.ts.");
  }

  const byRarity = cardRows.reduce<Record<string, number>>((acc, c) => {
