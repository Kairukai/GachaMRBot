  ...
  .addStringOption((o) =>
    o
      ...
      .addChoices(
        ...
        { name: "Cards burned", value: "burned" },
      ),
  );

const MEDALS = ["🥇", "🥈", "🥉"];

function formatScore(category: LeaderCategory, score: number): string {
  switch (category) {
    case "value":
      return `💠 ${score}`;
    case "cards":
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const category = (interaction.options.getString("category") ?? "value") as LeaderCategory;
