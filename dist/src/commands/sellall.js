...
import { confirmRow, ownedAtRarity } from "../lib/sell.js";
...
export const data = new SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: "Rare", value: "rare" }, { name: "Epic", value: "epic" }, { name: "Legendary", value: "legendary" }));
export async function execute(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(meta.color)
        .setDescription(`You are about to sell **every ${meta.label} card you own** in this server.\n\n` +
        `${listed}${overflow}`)
        .addFields({ name: "Cards", value: `${cards.length}`, inline: true }, { name: "Payout", value: `💠 ${total} (${each} each)`, inline: true }, {
        name: "That buys",
    ...
    if (rarity === "legendary") {
        embed.addFields({
            name: "🛑 Read this",
            value: "Legendaries average about 60 rolls each to obtain. Selling them is a " +
    ...
    return interaction.editReply({
        embeds: [embed],
        components: [
            confirmRow(`all:${rarity}`, `Sell ${cards.length} for 💠 ${total}`, rarity !== "rare"),
        ],
    });
}
